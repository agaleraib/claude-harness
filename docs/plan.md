...
- [x] Wave 25 - spec-planner strict acceptance-check discipline: shared deterministic scanner (`acceptance-strictness.sh`) → spec-planner self-check + `<P>/<T> strict` summary field + scanner-wired Codex reviewer criterion (`acceptance-review-focus.sh --emit-focus/--emit-log`); mechanical-abort layer dropped after 7-pass /planning-loop review. Cross-model code review at merge caught + fixed 4 fail-open/portability blockers the Opus-only dry-run build missed. Fixtures 74/74; dogfood strict=22/22 -> docs/waves/wave25-spec-planner-strict-acceptance.md (cdb80bb)
