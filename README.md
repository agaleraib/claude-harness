...
- Optional workflow skills you can reach for when you want structure
...
│   ├── session-end/                # Exit ritual + project-tracker
│   ├── micro/                      # Focused work blocks
│   ├── park/                       # Side-quest capture
...
├── procedures/
│   └── api-security-checklist.md   # API security reference
├── .harness-profile                # Project metadata (audience, stakes, stack)
├── parking_lot.md                  # Side-quests and deferred issues
├── scripts/
...
| Component | Location | Available In |
|---|---|---|
...
All skills are optional — invoke them when you want structure, skip them when you don't. They load on demand (zero context cost until invoked).
...
Checks for pending commits (offers to run `/commit`), captures state of play, triages parking lot, writes exit note, and saves session to Second Brain via project-tracker.
...
**Flow:**
1. Check for uncommitted changes → offer `/commit` or leave as-is
2. Show session summary
3. Ask how the day went (hit / partial / drifted / blocked)
4. Offer parking lot triage
5. Ask for tomorrow's first move
6. Write exit note
7. Run project-tracker automatically

#### `micro`

Frames a focused work block with one goal and a time budget. Keeps you from drifting into side-quests.

**When to use:** When starting a chunk of work and you want focus. Good for 10–90 minute blocks.
...
**When to use:** When you're ready to commit. Replaces raw `git commit` with a reviewed commit.

```
/commit
/commit "feat: add webhook retry logic"    # override commit message
```

**Flow:**
1. Check staged changes
2. Scan parking lot for items related to staged files — ask: resolve now or acknowledge?
3. Run code-reviewer on staged diff
4. If issues found → **fix all / review individually / park all**
5. Commit (max 2 re-review rounds)
6. Update `plan.md` if the commit moves a plan item forward
...
Caps at 3 rounds and never auto-ships. On `approve` → success path. On cap-reached `needs-attention` → Step 6.5 dispatches arbiters (`code-reviewer` for detail findings, `Plan` for scope findings), then either auto-applies (when arbiter rulings are unanimous + mechanical) or prints a 4-option menu with one option marked `(Recommended)` per Rule 12 (deterministic logic over verdict mix and stop-signals — wrong-premise → 4, 1-line implementation tweaks → 1, same-area regenerated → 3, default → 1). Auto-apply executes via temp-file + atomic rename with SHA-256 hash recheck and a per-finding JSON edit-block contract; you still own `/commit`. Opt out via `PLANNING_LOOP_NO_AUTO_APPLY=1` env var (per-shell) or `planning_loop.auto_apply: false` in `.harness-profile` (per-repo). Every round writes findings to `.harness-state/planning-loop/`.
...
**When to use:** After implementing a feature, fixing a bug, or before committing. Works on any code — backend, frontend, scripts, infrastructure. Also runs automatically as part of the `/commit` skill.
...
**When it runs:** Automatically at session-end. Also proactively before commits when meaningful work was done (new features, bug fixes, spec changes). Can be invoked explicitly.
...
|---|---|---|
| `.harness-profile` | Project metadata (type, audience, stakes, stack) | `project-init` |
| `parking_lot.md` | Side-quests and deferred issues | `park` (append), `session-end` (triage) |
| `plan.md` | Project plan | `spec-planner` (create), `commit` (update) |
| `.harness-state/last_exit.md` | Exit note from last session | `session-end` |
| `criteria/` | Quality rubrics for code-reviewer and ui-evaluator | `setup-harness` |
| `procedures/api-security-checklist.md` | API security reference checklist | `setup-harness` |
| `CLAUDE.md` | Build commands + non-obvious conventions | `setup-harness` |

---

## Setup

### 1. Install User-Level Components (once, applies everywhere)
...
You:   /commit
Claude: [code-review → fix/park → commit → plan.md updated]
```
...
### Workflow D: Full Day Session

```
You:   /session-start                    # context injection, pick today's goal
You:   /micro "wire the translate API"   # frame a focused block
       ... work ...
You:   /park "refactor the round loop"   # side-quest captured, keep going
       ... work ...
You:   /commit                           # review → commit → plan update
You:   /session-end                      # exit note, triage, save to Second Brain
```
...
The harness gives you better agents and criteria, but how you **manage sessions** determines whether Opus 4.7 performs at its best or degrades into confused loops. These practices come directly from [Anthropic's best practices](https://code.claude.com/docs/en/best-practices) and production experience.
...
**Caveat:** Avoid using Stop hooks for blocking signals — they fire on every turn and can create infinite loops with no escape hatch. Use hooks for deterministic automation (type-check, lint, format), not for advisory feedback.
