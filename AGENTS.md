...
**The gate is repo-resolved and fail-safe.** The mechanical gate is the entire reason it is safe to walk away, so a gate that fails OPEN is worse than no gate. The checks are **repo-resolved**: the TARGET repo declares its `tests`/`typecheck`/`verify` commands (the reference impl reads them from the target `.harness-profile` and threads them in; the engine stays parser-free). Each check is encoded in exactly one of two explicit forms — an argv list (run with no shell) or a shell scalar (run via `sh -c`); there is **no metacharacter sniffing**, and declaring a check in both forms is a config error. The gate is **fail-safe** (the three-way rule): no resolvable gate ⇒ the run is **refused before any work** AND each item is red per-item (never vacuously green); a configured-but-empty sub-check passes (a legitimate partial gate); a misconfigured gate fails closed → escalate. **The serial merge is fast-forward-only:** each item's branch is cut off HEAD and the per-item gate IS the merge gate (gated tree == merged tree on a fast-forward), so a green item fast-forwards HEAD with no re-gate; if HEAD diverged since the branch-cut, the merge refuses (`non-fast-forward`) and the item escalates rather than synthesizing an un-gated tree — and every gate execution's working-tree byproducts are discarded between items (a hard reset + clean that never removes ignored files).
