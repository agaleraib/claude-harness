...
12. **Every user-facing menu surfaces a Recommended option.** Any choice menu printed during `/planning-loop` execution — the cap-reached 4-option escalation menu, auto-apply abort menus, and any dynamic mid-loop menus surfaced by arbiter or sub-agents — MUST mark exactly one option `(Recommended)` with a one-line reason. The reason cites the verdict mix or stop-signal that drove the pick (e.g. "wrong-premise verdicts dominate", "defects are 1-line implementation tweaks", "same area regenerated findings across all 3 rounds"). Arbiter prompt templates (`references/codex-prompts.md` §3 detail-arbiter, §4 scope-arbiter) include a `Recommendation:` output field; the skill's static recommendation logic (SKILL.md Step 6 escalation-path block) is authoritative and overrides arbiter free-form suggestions when the static rule order has a higher-priority match (e.g. unanimous wrong-premise → option 4 regardless of arbiter's words). **Why:** cold-picking from an unrecommended menu wastes user attention on a decision the skill already has signal to make. Empirical anchor: round-10 menu observed 2026-05-10 surfaced 5 options without a recommendation, forcing the user to cold-pick — same finding-set the static logic would have routed to option 1 (per the design→implementation crossover signal in `feedback_planning_loop_stop_signal.md`).
